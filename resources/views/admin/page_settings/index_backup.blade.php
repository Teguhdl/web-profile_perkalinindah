...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="border-b border-gray-100">
        <nav class="flex -mb-px" aria-label="Tabs" id="tabs">
            ...
            <button onclick="switchTab('team')" id="tab-team" class="tab-btn py-4 px-4 text-center border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                Tim Kami (PDF)
            </button>
            <button onclick="switchTab('toggle')" id="tab-toggle" class="tab-btn py-4 px-4 text-center border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                ⚡ Toggle Section
            </button>
    ...
    <form action="{{ route('admin.page_settings.update') }}" method="POST" enctype="multipart/form-data" class="p-6">
        ...
        <!-- TOGGLE SECTION -->
        <div id="content-toggle" class="tab-content hidden">
            <h3 class="text-lg font-bold text-gray-900 mb-2">Toggle Section Dashboard</h3>
            <p class="text-sm text-gray-500 mb-6">Centang section yang ingin ditampilkan di halaman dashboard. Section yang tidak dicentang akan disembunyikan dari pengunjung.</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                @php
                    $toggles = [
                        ['key'=>'dashboard_show_hero', 'label'=>'Hero Section (Banner Utama)', 'icon'=>'🎯', 'desc'=>'Banner besar di paling atas dengan judul + tombol kontak.'],
                        ['key'=>'dashboard_show_about', 'label'=>'Tentang Kami', 'icon'=>'🏢', 'desc'=>'Section deskripsi perusahaan + 2 gambar.'],
                        ['key'=>'dashboard_show_video', 'label'=>'Video Profile', 'icon'=>'🎬', 'desc'=>'Section player video promosi.'],
                        ['key'=>'dashboard_show_products', 'label'=>'Produk Unggulan', 'icon'=>'📦', 'desc'=>'Grid 6 produk terbaru otomatis.'],
                        ['key'=>'dashboard_show_mitra', 'label'=>'Mitra Kami (Logo Scroller)', 'icon'=>'🤝', 'desc'=>'Logo mitra/partner scroll horizontal.'],
                    ];
                @endphp

                @foreach($toggles as $t)
                    <label class="flex items-start gap-4 p-5 bg-white border-2 border-gray-200 rounded-xl cursor-pointer hover:border-red-300 transition has-[:checked]:border-red-500 has-[:checked]:bg-red-50">
                        <input type="checkbox" name="{{ $t['key'] }}" value="1" {{ ($settings[$t['key']] ?? '1') == '1' ? 'checked' : '' }} class="w-6 h-6 mt-1 accent-red-600 rounded">
                        <div class="flex-1">
                            <div class="font-bold text-gray-900 flex items-center gap-2">{{ $t['icon'] }} {{ $t['label'] }}</div>
            ...
            <div class="mt-8 bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg max-w-4xl">
                <div class="flex items-start gap-3">
                    <span class="text-blue-500 text-xl">💡</span>
                    <div class="text-sm text-blue-700">
                        <strong>Tips SEO:</strong> Jangan hide section "Tentang Kami" karena ini penting untuk keyword optimization & local SEO. Section "Hero" sebaiknya selalu aktif untuk first impression pengunjung.
                    </div>
                </div>
            </div>
        </div>
